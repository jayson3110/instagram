...
export default function AddComment({docId, comments, setComments,commentInput}) {
	...
	const handSubmitComment = (event) => {
		...
		// then we have new array with the new comment and the older comments
	...
	return <div className="border-t border-gray-primary">
	     ...
	     method="POST" onSubmit={(event) => comment.length >= 1 ? handSubmitComment(event):
	     event.preventDefault()}>
	         ...
	         <button className={`text-sm font-bold text-blude-medium ${!comment && 'opacity-25'}`}
	                 type="button"
